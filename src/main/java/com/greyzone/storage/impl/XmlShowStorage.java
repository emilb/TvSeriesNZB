...
public class XmlShowStorage implements Storage<Show> {
	...
	public List<Show> getItems() {
		...
		try {
			FileInputStream fis = new FileInputStream(
					appSettings.getShowsFileName());
			TvSeriesNzb t = (TvSeriesNzb) x.fromXML(fis);
			fis.close();
			return t.getShows();
		} catch (Exception e) {
			log.error(
					"Could not read or parse " + appSettings.getShowsFileName(),
					e);
	...
	public void storeItems(List<Show> shows) {
		...
		try {
			log.debug("Trying to save updated "
					+ appSettings.getShowsFileName());
			...
			log.error(
					"Failed to write to file: "
							+ appSettings.getShowsFileName(), e);
		}
	}

	@Override
	public void testParsing() {
		log.info("Testing " + appSettings.getShowsFileName()
				+ " correctness...");
		XStream x = new XStream();
		x.processAnnotations(TvSeriesNzb.class);
		...
		log.info("... file " + appSettings.getShowsFileName()
